...
def training_loop(train_dataloader, opts):
    ...
    for epoch in range(opts.num_epochs):
        ...
        for batch in train_dataloader:
            ...
            D_real_sum = torch.sum(torch.pow(D(real_images) - 1, 2))
            D_real_loss = D_real_sum / (2 * opts.batch_size)
            ...
            D_fake_sum = torch.sum(torch.pow(D(fake_images), 2))
            D_fake_loss = D_fake_sum / (2 * opts.batch_size)
            ...
            G_sum = torch.sum(torch.pow(D(fake_images) - 1, 2))
            G_loss = G_sum / opts.batch_size
